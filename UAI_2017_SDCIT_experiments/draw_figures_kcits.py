...
names_kcit3_chaotic = ['independent', 'gamma', 'trial', 'N', 'boot_p_value']
# names_kcit3_postnonlinear = ['sigma_squared', 'independent', 'noise', 'trial', 'N', 'boot_p_value']
names = {
    ('KCIT', 'chaotic'): names_kcit_chaotic,
    ('KCIT2', 'chaotic'): names_kcit_chaotic,
    ('KCIT5', 'chaotic'): names_kcit3_chaotic,
}

pvalue_column = collections.defaultdict(lambda: 'boot_p_value')  # {'KCIT': 'pvalue', 'KCIT': 'boot_p_value', 'SDCIT': 'pvalue', 'CSDCIT': 'pvalue', 'KCIPT': 'pvalue', 'KCIT2': 'pvalue', 'KCIT2': 'boot_p_value'}
color_palettes = sns.color_palette('Paired', 10)
method_color_codes = {}  # {'KCIT': 0, 'KCIT2': 2, 'KCIT3': 4, 'KCIT4': 6}
markers = collections.defaultdict(lambda: 'o')  # {'KCIT': '^', 'KCIT': 'o', 'SDCIT': 's', 'KCIPT': '*', 'CSDCIT': '<', 'KCIT2': '>', 'KCIT2': 'o'}
all_algos = ['KCIT', 'KCIT2', 'KCIT5']
...
def draw_aupc_chaotic():
    ...
    plt.savefig(SDCIT_FIGURE_DIR + '/{}_aupc_kcits.pdf'.format(data), transparent=True, bbox_inches='tight', pad_inches=0.02)
    plt.close()
...
def draw_calib_chaotic():
    ...
    handles, labels = plt.axes().get_legend_handles_labels()
    plt.axes().legend(handles[::-1], labels[::-1])
    plt.savefig(SDCIT_FIGURE_DIR + '/chaotic_calib_kcits.pdf', transparent=True, bbox_inches='tight', pad_inches=0.02)
    plt.close()
...
def draw_aupc_postnonlinear():
    ...
    for k, gdf in aupc_df.groupby(['algorithm', 'N']):
        ...
        if k[1] == 400:
            plt.plot(gdf['dimension'], gdf['AUPC'], markers[(k[0])], c=color_palettes[method_color_codes[k[0]]] if k[1] == 400 else color_palettes[-0 + method_color_codes[k[0]]], ls='-' if k[1] == 400 else ':', label=str(k[0]))
        else:
