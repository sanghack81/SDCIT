...
from UAI_2017_SDCIT_experiments.testing_utils import chaotic_configs, read_chaotic, read_postnonlinear_noise
...
def test_chaotic(independent, gamma, trial, N, sigma_squared):
    ...
    pval = FCIT_K(KX, KY, KZ, seed=trial, with_gp=False, eq_17_as_is=True, sigma_squared=sigma_squared)
...
def main():
    with Parallel(12) as parallel:
        if not exists(SDCIT_RESULT_DIR + '/fcit3_chaotic.csv'):
            for independent, N, gamma in tqdm(chaotic_configs()):
                for sigma_squared in [1.0, 0.1, 0.01, 0.001]:
                    ...
                    with open(SDCIT_RESULT_DIR + '/fcit3_chaotic.csv', 'a') as f:
                        for out in outs:
                            print(sigma_squared, *out, sep=',', file=f, flush=True)
        else:
            print('skipping FCIT on chaotic time series data')

            # if not exists(SDCIT_RESULT_DIR + '/fcit3_postnonlinear.csv'):
            ...
            #             with open(SDCIT_RESULT_DIR + '/fcit3_postnonlinear.csv', 'a') as f:
            #                 for out in outs:
            #                     print(sigma_squared, *out, sep=',', file=f, flush=True)
            # else:
