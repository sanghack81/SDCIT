...
def main():
    with Parallel(24) as parallel:
        if not exists(SDCIT_RESULT_DIR + '/kcitk_chaotic.csv'):
            for independent, N, gamma in tqdm(chaotic_configs()):
                ...
                with open(SDCIT_RESULT_DIR + '/kcitk_chaotic.csv', 'a') as f:
                    for out in outs:
                        print(*out, sep=',', file=f, flush=True)
        else:
            print('skipping KCITK on chaotic time series data')

        if not exists(SDCIT_RESULT_DIR + '/kcitk_postnonlinear.csv'):
            for noise, independent, N in tqdm(postnonlinear_noise_configs()):
                ...
                with open(SDCIT_RESULT_DIR + '/kcitk_postnonlinear.csv', 'a') as f:
                    for out in outs:
                        print(*out, sep=',', file=f, flush=True)
        else:
            print('skipping KCITK on post nonlinear noise data')
