...
def test_chaotic(independent, gamma, trial, N):
    ...
    pval = python_kcit(*read_chaotic_data(independent, gamma, trial, N), seed=trial, with_gp=True)[2]
    return independent, gamma, trial, N, pval


def test_postnonlinear(independent, noise, trial, N):
    np.random.seed(trial)
    pval = python_kcit(*read_postnonlinear_noise_data(independent, noise, trial, N), seed=trial, with_gp=True)[2]
    return independent, noise, trial, N, pval


def main():
    with Parallel(7) as parallel:
        if not exists(SDCIT_RESULT_DIR + '/kcitr_chaotic.csv'):
            for independent, N, gamma in tqdm(chaotic_configs()):
                ...
                with open(SDCIT_RESULT_DIR + '/kcitr_chaotic.csv', 'a') as f:
                    for out in outs:
                        print(*out, sep=',', file=f, flush=True)
        else:
            print('skipping KCITR on chaotic time series data')

        if not exists(SDCIT_RESULT_DIR + '/kcitr_postnonlinear.csv'):
            for noise, independent, N in tqdm(postnonlinear_noise_configs()):
                ...
                with open(SDCIT_RESULT_DIR + '/kcitr_postnonlinear.csv', 'a') as f:
                    for out in outs:
                        print(*out, sep=',', file=f, flush=True)
        else:
            print('skipping KCITR on post nonlinear noise data')
