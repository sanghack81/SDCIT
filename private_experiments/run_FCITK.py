...
def main():
    with Parallel(8) as parallel:
        if not exists(SDCIT_RESULT_DIR + '/fcitk_chaotic.csv'):
            for independent, N, gamma in tqdm(chaotic_configs()):
                ...
                with open(SDCIT_RESULT_DIR + '/fcitk_chaotic.csv', 'a') as f:
                    for out in outs:
                        print(*out, sep=',', file=f, flush=True)
        else:
            print('skipping FCIT on chaotic time series data')

        if not exists(SDCIT_RESULT_DIR + '/fcitk_postnonlinear.csv'):
            for noise, independent, N in tqdm(postnonlinear_noise_configs()):
                ...
                with open(SDCIT_RESULT_DIR + '/fcitk_postnonlinear.csv', 'a') as f:
                    for out in outs:
                        print(*out, sep=',', file=f, flush=True)
        else:
