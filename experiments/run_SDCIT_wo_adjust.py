...
def test_chaotic(independent, gamma, trial, N):
    ...
    mmsd, pval = SDCIT(*read_chaotic(independent, gamma, trial, N), seed=trial, adjust=False)
    return independent, gamma, trial, N, mmsd, pval


def test_postnonlinear(independent, noise, trial, N):
    np.random.seed(trial)
    mmsd, pval = SDCIT(*read_postnonlinear_noise(independent, noise, trial, N), seed=trial, adjust=False)
    return independent, noise, trial, N, mmsd, pval


def main():
    with Parallel(24) as parallel:
