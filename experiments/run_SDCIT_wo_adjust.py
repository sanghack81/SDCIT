...
def test_chaotic(independent, gamma, trial, N):
    ...
    mmsd, pval = SDCIT(*read_chaotic(independent, gamma, trial, N), seed=trial, adjust=False, to_shuffle=False)
    return independent, gamma, trial, N, mmsd, pval


def test_postnonlinear(independent, noise, trial, N):
    np.random.seed(trial)
    mmsd, pval = SDCIT(*read_postnonlinear_noise(independent, noise, trial, N), seed=trial, adjust=False, to_shuffle=False)
    return independent, noise, trial, N, mmsd, pval


def main():
    with Parallel(PARALLEL_JOBS) as parallel:
        if not exists(SDCIT_RESULT_DIR + '/sdcit-wo-adjust_chaotic.csv'):
            for independent, N, gamma in tqdm(chaotic_configs()):
                ...
                with open(SDCIT_RESULT_DIR + '/sdcit-wo-adjust_chaotic.csv', 'a') as f:
                    for out in outs:
                        print(*out, sep=',', file=f, flush=True)
        else:
            print('skipping SDCIT on chaotic time series data')

        if not exists(SDCIT_RESULT_DIR + '/sdcit-wo-adjust_postnonlinear.csv'):
            for noise, independent, N in tqdm(postnonlinear_noise_configs()):
                ...
                with open(SDCIT_RESULT_DIR + '/sdcit-wo-adjust_postnonlinear.csv', 'a') as f:
                    for out in outs:
                        print(*out, sep=',', file=f, flush=True)
        else:
