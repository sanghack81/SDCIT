...
% Sanghack Lee ------------------- theta = 0 will use 'median heuristic'
...
% Kx = kernel2([x z], [x z], [theta,1]); Kx = H * Kx * H;
Kx = kernel2([x z/2], [x z/2], [theta,1]); Kx = H * Kx * H;
% Ky = kernel2([y z], [y z], [theta,1]); %Ky = Ky * H;
% Kx = kernel2([x], [x], [theta,1]); %Kx = Kx * H; %%%%Problem
Ky = kernel2([y], [y], [theta,1]); Ky = H * Ky * H;  %%%%Problem
...
if IF_GP
    ...
    Kz = kernel2(z, z, [theta,1]); Kz = H * Kz * H; %*4 % as we will calculate Kz
    % Kernel matrices of the errors
