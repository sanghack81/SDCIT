...
def test_kcit_henon():
    ...
    for gamma in [0.0, 0.3, 0.5]:
        ...
        aupc_gamma = aupc(ps)
        if gamma==0.0:
            assert 0.45<=aupc_gamma<=0.55   # 0.47736
        if gamma==0.3:
            assert 0.85<=aupc_gamma<=0.925 # 0.902876
        if gamma==0.5:
            assert 0.85<=aupc_gamma # 1.0
...
def test_matlab_kcit_henon():
    ...
    for gamma in [0.0, 0.3, 0.5]:
        ...
        aupc_gamma = aupc(ps)
        if gamma==0.0:
            assert 0.45<=aupc_gamma<=0.55   # 0.483944
        if gamma==0.3:
            assert 0.85<=aupc_gamma<=0.925 # 0.878246
        if gamma==0.5:
            assert 0.85<=aupc_gamma # 0.874698
        print(gamma, aupc_gamma)
