...
from sdcit.utils import centering, pdinv, truncated_eigen, eigdec, columnwise_normalizes, residual_kernel, rbf_kernel_with_median_heuristic
...
def python_kcit(x: np.ndarray, y: np.ndarray, z: np.ndarray, alpha=0.05, with_gp=True, noise=1e-3, num_bootstrap_for_null=5000, normalize=False, kern=rbf_kernel_with_median_heuristic, seed=None):
    if seed is not None:
        np.random.seed(seed)
    T = len(y)
...
def python_kcit_K(Kx: np.ndarray, Ky: np.ndarray, Kz: np.ndarray, alpha=0.05, with_gp=True, noise=1e-3, num_bootstrap_for_null=5000, seed=None):
    ...
    if with_gp:
        ...
        P = eye(T) - Kz @ pdinv(Kz + noise * eye(T))
        Kxz = P @ Kx @ P.T
        Kyz = P @ Ky @ P.T
...
def null_by_gamma_approx(statistic, alpha, uu_prod):
...
def null_by_bootstrap(statistic, num_bootstrap, alpha, eig_uu):
